 ...
 */
fun SignalUser.encryptFor(recipient: SignalUser, plaintext: String): EncryptedEnvelope {
    val cipher = SessionCipher(store, recipient.address)
...
fun SignalUser.decryptFrom(sender: SignalUser, envelope: EncryptedEnvelope): String {
    val cipher = SessionCipher(store, sender.address)
