...
**Volcano / iterator model** (classic row-store execution): every operator (scan, filter, join) implements a `next()` that returns one row at a time, pulled up through the whole operator tree. Each row pays the full cost of dispatching through every operator in the chain — and because operators are polymorphic (the query plan is built from generic types at runtime), each `next()` call is a **virtual function call**: a pointer dereference through a vtable to find the actual code address, rather than a direct jump to known code. This indirect jump is hard for the CPU's **branch predictor** to guess correctly (unlike a normal `if/else`, which predicts well), so mispredictions repeatedly stall the CPU's instruction pipeline — multiplied across millions of rows. The resulting loop shape is also too irregular (unpredictable branches, indirect calls) for the compiler to **vectorize** — i.e., to emit SIMD instructions that process multiple values per instruction.
...
**Note that vectorization alone isn't DuckDB's differentiator** — ClickHouse, Snowflake, and BigQuery all use vectorized execution too; it's well-established technique dating to the mid-2000s. Older row-store engines (Postgres, MySQL, Oracle) haven't retrofitted it not because it's not worth it, but because it requires being designed in from the ground up — rewriting a mature engine's storage/executor foundation is a different order of undertaking than adding a feature. DuckDB's actual differentiator is combining vectorized/columnar execution with an **embedded deployment model** nobody else paired it with (see below).

---

## Embedded / In-Process Architecture

DuckDB has no server process — it's a library loaded directly into your application's address space, same deployment story as SQLite. Compare to a client-server database (even over `localhost`): every query still crosses a real boundary — `send()`/`recv()` syscalls (context switches into the kernel), the OS copying data into/out of socket buffers, the server process being scheduled, and results being **serialized** into a wire protocol on the way out and **deserialized** back into usable objects on the way in. For an OLTP query returning a handful of rows this is trivial; for an analytical query returning millions of rows into a DataFrame, that serialization/copy tax can dominate total query time.
