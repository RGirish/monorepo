...
Note: content generated from an interactive teach-then-implement session (no scratch file was hand-written — `wiki/scratch/week-18-2026-05-04-build.md` was created but left with only its header filled in). Started with a broad survey of embedding compression technique families (scalar quantization, PQ/OPQ, binary quantization/hashing, dimensionality reduction, Matryoshka Representation Learning, and how they compose), researched via a background agent grounded in current vendor docs and papers rather than prior knowledge — filed as synthesis/embedding-compression-techniques.md since it's a comparison worth keeping beyond this one build. Only int8 scalar quantization was actually implemented this week (both asymmetric min/max and symmetric zero-centered variants), including a worked row-by-row numeric trace of one vector through the full quantize/dequantize pipeline, and a demonstration that symmetric quantization lets similarity search run directly on compressed int8 values via one scalar correction factor. The other technique families remain a mapped-but-unimplemented landscape for future weeks.
[lint] No orphans: embedding-vector-quantization.md has inbound links from week-18, index.md, embedding-models.md, and vector-db.md, with outbound links back to all of them plus the new synthesis page. embedding-compression-techniques.md has inbound links from the build page and index.md. Missing links: none found elsewhere referencing scalar/int8 quantization. Concept gap: quantization/compression tradeoffs currently appear on only 1 build page — not yet promoted to concepts/, watching for a second occurrence (e.g. if a future week implements PQ or binary quantization) before creating a shared concepts/ page. Stale content: none found — backlog.md had no matching item to remove.
...
Addendum (same day): human asked for verification that all 6 lesson stages and every follow-up question's answer actually made it into tools/duckdb.md. Found the Volcano-model walkthrough had been condensed too far — expanded it back out to the concrete `SELECT ... WHERE region='US'` operator-chain trace, the vtable/branch-prediction mechanics, and the vectorizable-loop code example; also restored the SQLite-is-row-oriented / Postgres-extensions nuance behind "what's actually special about DuckDB," and the row-oriented-wire-protocol detail behind the embedded-architecture serialization cost. No other gaps found against the transcript.
...
Note: content for this entry was generated entirely from an interactive Q&A session (no scratch file was hand-written — `wiki/scratch/week-18-2026-05-04-ai-learning.md` exists but was left with its `Topic:` field blank) working through Anthropic's and Educative's prompt engineering guides, plus a follow-up survey of advanced techniques (ToT/GoT, self-consistency, ReAct, Reflexion, APE, multi-agent debate) and which require external orchestration vs. a single prompt. No build this week — `build: none` in frontmatter, consistent with the week-13 precedent for AI-only weeks.
[lint] No orphans: prompt-engineering.md has inbound links from week-18, index.md, and context-engineering.md, with outbound links to context-engineering.md and language-modeling-fundamentals.md, both now updated with reciprocal inbound links. Missing links: language-modeling-fundamentals.md's next-token-prediction section had no forward reference to how that mechanism is exploited by CoT/prefilling — added. Concept gap: the autoregressive-generation-as-compute-budget idea now appears on 2 pages (language-modeling-fundamentals.md, prompt-engineering.md) but is treated as a shared reference rather than spun into its own concepts/ page yet — watching for a third occurrence before promoting it. Stale content: none found. Separately flagged to the human — CLAUDE.md's Week Reference table has an off-by-one date error starting at week 19 (duplicates week 18's date, then continues one week early through week 52); README's not-started rows for weeks 19–32 were corrected here, but the source table itself is untouched pending the human's call on how to fix it.
...
## [2026-09-04] ingest | week-20 build — ray-word-length-pipeline
Created: builds/ray-word-length-pipeline.md, tools/ray.md, code/distributed-systems/ray-word-length-pipeline/ (generate_data.py, baseline_sequential.py, parallel_ray_tasks.py, parallel_with_stats_actor.py, fault_tolerance.py)
Updated: weeks/week-20-2026-05-18.md (frontmatter `build` field, previously "none"; tags extended; Build section added), concepts/concurrency-and-atomicity.md (4th row + Common Thread paragraph for Ray's actor-serialization strategy), synthesis/ray-tasks-actors-and-spark-comparison.md (GIL-vs-multiprocessing clarification, empirical heterogeneous-core straggler finding, new Fault Tolerance section, Related pages backfilled), tools/duckdb.md (cross-link to tools/ray.md for contrast), index.md, README.md (progress table row 20, Best builds, new Concurrency-and-atomicity running theme), log.md
Note: build content generated entirely from an interactive teach-then-implement session, following the Build Sessions workflow (orient → propose 6-stage plan → implement one stage at a time with a concept-probing question between each). Human explicitly asked, mid-build, that code always be shown inline in chat with a walkthrough rather than just written to disk — codified as a new standing rule in CLAUDE.md's "## Build Sessions" (step 6), superseding what had only been a memory-file preference. Human also asked to ensure all Q&A learning content from the session was captured in synthesis pages before ingest — resulted in substantial expansion of synthesis/ray-tasks-actors-and-spark-comparison.md (originally filed pre-build) with content that only emerged during the build itself (GIL correction, the M4 P-core/E-core straggler effect measured empirically, and the full fault-tolerance retries-vs-real-fixes section).
[lint] No orphans: ray-word-length-pipeline.md and tools/ray.md both have inbound links from week-20, index.md, README.md, and each other, with outbound links to the synthesis page and concepts/concurrency-and-atomicity.md, both of which now link back. Missing links: none found elsewhere referencing Ray/distributed task-actor concepts. Concept gap: concurrency-and-atomicity.md now spans 4 builds (weeks 4, 9, 19, 20) — already an established concepts page from week 19, simply extended rather than requiring a new one. Stale content: none found — backlog.md had no matching Ray/distributed-computing item to remove; the concurrency-and-atomicity.md intro line was updated from "Three builds" to "Four builds" to stay accurate.

## [2026-09-04] query | Ray fundamentals — tasks vs. actors, scheduling, and Ray vs. Spark
Ahead of the week 20 build (a distributed data aggregation pipeline with Ray), worked through: tasks as stateless-but-chainable via `ObjectRef` data dependencies (forming an implicit DAG, not truly "independent"); actors as one-process-per-instance with private state, not shared across actor instances; Ray's scheduler as decentralized/greedy — dependency-driven eligibility plus locality-aware placement, not a global upfront optimizer like Spark's Catalyst; and a Ray vs. Spark vs. Dask decision guide. Filed as synthesis/ray-tasks-actors-and-spark-comparison.md.
Updated: index.md (new synthesis entry).

## [2026-09-03] ingest | week-19 build — redis-rate-limiter
Created: builds/redis-rate-limiter.md, concepts/concurrency-and-atomicity.md
Updated: weeks/week-19-2026-05-11.md (build field filled in, build section added), builds/two-phase-commit.md (cross-linked to redis-rate-limiter.md and new concepts page), builds/crdt-collaborative-editor.md (same), index.md, README.md (progress table row, Best builds), log.md
[lint] Filed concepts/concurrency-and-atomicity.md immediately rather than deferring — the theme (multi-step-operation atomicity under concurrency) now spans 3 build pages (two-phase-commit week 4, crdt-collaborative-editor week 9, redis-rate-limiter week 19), clearing the 2+ page concept-gap threshold. All three build pages now cross-link to the new concepts page and to each other. Redis Rate Limiter ingested from live conversation content; build scratch file (week-19-2026-05-11-build.md) was left with only the "What I'm building" line filled in — ingested from conversation per updated CLAUDE.md policy. Week 20's build remains outstanding (build: none), untouched by this ingest.
