...
- [Embeddings] Binary quantization / hashing for embedding vectors (Week 18 follow-up) — implement sign-bit binary quantization and RaBitQ, compare recall and compression ratio against Week 18's int8 scalar quantization; see [Embedding Compression Techniques](synthesis/embedding-compression-techniques.md) family 3
- [Embeddings] Product Quantization (PQ/OPQ) for embedding vectors (Week 18 follow-up) — implement codebook-based compression via k-means (and OPQ's rotation step), compare against scalar quantization at matched compression ratios; see [Embedding Compression Techniques](synthesis/embedding-compression-techniques.md) family 2
- [Embeddings] Dimensionality reduction + quantization composition (Week 18 follow-up) — apply PCA (with a de-skew rotation) before scalar quantization, measure the compounding compression/accuracy tradeoff vs. quantization alone; see [Embedding Compression Techniques](synthesis/embedding-compression-techniques.md) family 4
- [Embeddings] Matryoshka truncation + quantization + rerank pipeline (Week 18 follow-up) — using an MRL-trained embedding model, combine dimension truncation with int8/binary quantization and a reranking step, and try to replicate the vendor-reported ~78% storage reduction; see [Embedding Compression Techniques](synthesis/embedding-compression-techniques.md) families 5–6
